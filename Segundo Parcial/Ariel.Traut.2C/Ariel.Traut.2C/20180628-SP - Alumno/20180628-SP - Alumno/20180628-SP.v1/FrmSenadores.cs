...
namespace _20180628_SP.v1
{
    public partial class FrmSenadores : Form
    {
        ...
        public void ManejadorVoto(string senador, Votacion.EVoto voto)
        {
            ...
            else
            {
                ...
                if (aux == 0)
                {
                    ...
                    // Guardar resultados                  
                    DAO sqlGuardar = new DAO();
                    sqlGuardar.Guardar("Votaciones", votacion);
                    SerializarBinaria binaryGuardar = new SerializarBinaria();
                    binaryGuardar.Guardar(Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + Path.DirectorySeparatorChar + "Votacion.txt", votacion);
                }
            }
        }
        ...
        private void btnSimular_Click(object sender, EventArgs e)
        {
            ...
            // THREAD
            Thread hiloSimular = new Thread(votacion.Simular);
            hiloSimular.Start();
        ...
        {
            string path = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + Path.DirectorySeparatorChar + "Votacion.txt";
            if(File.Exists(path))
            {
                SerializarBinaria binaryLeer = new SerializarBinaria();
                binaryLeer.Leer(path);
            }
        }
    }
}
